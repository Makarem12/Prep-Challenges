...
//  [5, 6, 7, 7]
// 
const y=[5.4, 5.5 ,6.7, 6.8] 
const roundDecimals = (arr) => {
    let round1=[]
    arr.forEach((item) => {
       let roundedItem = Math.round(item)
        round1.push(roundedItem);
      });
      return round1;
}
// console.log(roundDecimals(y))
// -------------------------------------------------------------------------------------------------------
...
const employeesBonus = (arr) => {
    let updatedData=[]
    arr.forEach((item) => {
        let salaryNumber = parseInt(item.salary)
        if(item.workHours>8){
            salaryNumber+=100
        }else{
           salaryNumber+=50
        }
        item.salary = salaryNumber + "$";
